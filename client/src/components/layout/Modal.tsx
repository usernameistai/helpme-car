...
const Modal: FC<ModalProps> = ({ isOpen, title = "Confirm", message, onConfirm, onCancel }) => {
  ...
  return createPortal(
    <>
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 dark:bg-black/50 backdrop-blur-sm">
        <div className="relative bg-white/10 backdrop-blur-md border border-white/30 rounded-2xl shadow-xl max-w-md md:w-full p-6 text-center transform transition-all animate-scale-up hover:shadow-[0_20px_50px_rgba(34,211,238,0.3),inset_5px_5px_10px_rgba(255,255,255,0.2)] right-4 md:right-0">
          ...
          <div className="flex justify-end gap-4 font-semibold font-poppins">
            <button
              onClick={onCancel}
                className='bg-gray-500/20 text-gray-400 border-2 border-gray-500/50 px-5 py-2 rounded shadow-lg transition-all transform hover:translate-y-[0.03rem] hover:shadow-[inset_0px_1px_15px_rgba(0,0,0,0.2)] '
            >
              Cancel
            </button>
            <button
              onClick={onConfirm}
                className='bg-red-500/20 text-red-400 border-2 border-red-500/50 px-5 py-2 rounded shadow-lg transition-all transform hover:translate-y-[0.03rem] hover:shadow-[inset_0px_1px_15px_rgba(0,0,0,0.2)] '
            >
              Delete
            </button>
          </div>
